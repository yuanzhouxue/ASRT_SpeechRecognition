...
def GetFrequencyFeature(wavsignal, fs):
    if(16000 != fs):
        raise ValueError(
            '[Error] ASRT currently only supports wav audio files with a sampling rate of 16000 Hz, but this audio is ' + str(fs) + ' Hz. ')
...
def getSTFTFeature(wavsignal, fs):
    import tensorflow as tf
    frame_length_ms = 25
    frame_step_ms = 10
    frame_length = int(frame_length_ms * fs / 1000)
    frame_step = int(frame_step_ms * fs/1000)
    stft_feature = abs(tf.signal.stft(wavsignal[0].astype(np.float), frame_length, frame_step))
    stft_log = tf.math.log(stft_feature)
    return stft_log.numpy()[:, :200]
...
def GetFrequencyFeature4(wavsignal, fs):
    ...
    data_input = np.zeros((range0_end, window_length // 2),
                          dtype=np.float)  # 用于存放最终的频率特征数据
...
if(__name__ == '__main__'):
    ...
    freimg = getSTFTFeature(wave_data, fs)
    t1 = time.time()
